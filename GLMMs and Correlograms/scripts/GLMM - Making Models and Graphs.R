...
plot.rem.2018.legend <-
  ...
  scale_shape_manual(name= "Seed Fate",
                     values = c(19, 17, 15, 18),
                     labels=c("Dispersed", "Intact", "Eaten", "Lost"))+
  ##setting collors
  scale_color_manual(name= "Seed Fate", 
                     values=c("chartreuse4", "deepskyblue", "firebrick2", "yellow3"),
                     labels=c("Dispersed", "Intact", "Eaten", "Lost")
...
plot.retripred.2017 <- 
  ...
  scale_shape_manual(name= "Seed Fate",
                     values = c(19, 17, 15, 18),
                     labels=c("Dispersed", "Intact", "Eaten (seed)", "Eaten (cache)"))+
  ##setting collors
  scale_color_manual(name= "Seed Fate", 
...
Nearlydispersal.2018 <- read.csv("dados/cutiext_2018.csv")
...
Nearly.dispersal2018 <- read.csv("dados/cutiext_2018.csv")
...
Np.dispersal2017 <- Np.dispersal2017 %>% select(-X,-X.1,-X.2,-N) %>% 
  mutate(Year="2017")
Np.dispersal2018 <- Np.dispersal2018 %>% select(-X,-X12,-X13,-log30,-log60,-log120,
...
Nearly.dispersal2017 <- Nearly.dispersal2017 %>% select(-X,-X.1,-X.2,-N) %>% 
  mutate(Year="2017")
Nearly.dispersal2018 <- Nearly.dispersal2018 %>% select(-X,-X12,-X13,-log30,-log60,-log120,
                                                -Cento_e_vinte,-Trinta) %>% 
  mutate(Year="2018")
